...
from sklearn.preprocessing import OrdinalEncoder  # 안정적
...
def main():
    ...
    parser.add_argument("--task", type=str, default="user-visits")
    parser.add_argument("--schema_path_gt", type=str, default="rel-avito/avito_schema.txt")
    parser.add_argument("--stats_path", type=str, default="rel-avito/feature_avito.txt")
    parser.add_argument("--task_path", type=str, default="rel-avito/task-user-visits.txt")
    parser.add_argument("--entity_table", type=str, default="UserInfo")
    parser.add_argument("--entity_col", type=str, default="UserID")
    ...
    parser.add_argument(
        "--models",
        type=str,
        default="claude-sonnet-4-20250514, claude-haiku-4-5-20251001, claude-3-7-sonnet-20250219",
    ...
    target_col: str = "num_click",
    drop_cols: tuple = ("UserID", "timestamp"),
    sample_frac: float = 0.3,
    entity_col_local: str = "UserID"
) -> dict:
        ...
        train_df[num_cols_all] = train_df[num_cols_all].fillna(0)
        valid_df[num_cols_all] = valid_df[num_cols_all].fillna(0)

        train_df = train_df.sample(frac=sample_frac, random_state=seed)

        X_train = train_df.drop(columns=list(drop_cols) + [target_col], errors="ignore").copy()
        y_train = train_df[target_col].astype(int)
        X_val   = valid_df.drop(columns=list(drop_cols) + [target_col], errors="ignore").copy()
        ...
        if categorical_cols:
            for col in categorical_cols:
                ...
                X_val[col] = X_val[col].astype(str).map(mapping).astype('float64')  # NaN 허용
        ...
        lgb_tr  = lgb.Dataset(X_train, label=y_train, categorical_feature=categorical_cols)
        lgb_va  = lgb.Dataset(X_val,   label=y_val,   categorical_feature=categorical_cols, reference=lgb_tr)
    ...
    train_csv_ori_path = f"data/{dataset}/{task}/{task}-train-ori.csv"
    valid_csv_ori_path = f"data/{dataset}/{task}/{task}-val-ori.csv"
    test_csv_ori_path  = f"/data/{dataset}/{task}/{task}-test-ori.csv"
...
Goal: Based on the responses from Step 1 (Tables) and Step 2 (Columns), decide which aggregation functions to apply in order to generate up to three new features that augment the target table {entity_table}.

--- Response from Step 1.1 ---
{block_to_str(response_step1)}
------

--- Response from Step 1.2 ---
...
"""
        ...
        # Refer to Schema for exact column names.
        ...
        os.makedirs("data/Claude/{dataset}/{task}/", exist_ok=True)
...
"""
        ...
        output_file = f'../{args.dataset}_{args.task}{args.n_iter}-iter{i}.py'
    ...
    # Step 3.2 Validation-based Feature Filtering
        kept_in_iter = False
        for k in range(1, 4):
