...
def main():
    ...
    parser.add_argument("--dataset", type=str, default="data/rel-event")
    parser.add_argument("--task", type=str, default="data/user-repeat")
    parser.add_argument("--schema_path_gt", type=str, default="data/rel-event/event_schema.txt")
    parser.add_argument("--stats_path", type=str, default="data/rel-event/feature_event.txt")
    parser.add_argument("--task_path", type=str, default="data/rel-event/task-user-repeat.txt")
    parser.add_argument("--entity_table", type=str, default="users")
    parser.add_argument("--entity_col", type=str, default="user_id")
    parser.add_argument("--round", type=int, default=1)
    parser.add_argument("--temp", type=float, default=0.2)
    parser.add_argument("--n_iter", type=int, default=3)
    parser.add_argument("--delta", type=float, default=0.0)

    # NEW: 멀티 LLM + LLM 투표 옵션
    parser.add_argument(
        "--models",
        type=str,
    ...
    def check_feature_k(
        ...
        drop_cols: tuple = ('index', 'timestamp', 'user_id', 'joinedAt', 'location'),
        ...
        categorical_cols = ['locale', 'gender']
    ...
    ui_train.rename(columns={'user':'user_id'}, inplace=True)
    ui_valid = task_relbench.get_table("val", mask_input_cols=False).df
    ui_valid.rename(columns={'user':'user_id'}, inplace=True)
    ui_test = task_relbench.get_table("test", mask_input_cols=False).df
    ui_test.rename(columns={'user':'user_id'}, inplace=True)
...
"""
        ...
        ui_val.rename(columns={{'user':'user_id'}}, inplace=True)
        ui_test = task.get_table("test", mask_input_cols=False).df
        ui_test.rename(columns={{'user':'user_id'}}, inplace=True)


        timemax_trn = ui_train["timestamp"].max() #**DO NOT MODIFY THIS LINE**
        timemax_val = ui_val["timestamp"].max()
        timemax_tst = ui_test["timestamp"].max()
        ...
        try:
            ...
            df_trn1.to_csv("../../../data/kyungho/Claude3.7/{dataset}/{task}/1116/{task}-train-{i}-1.csv", index=False)
            df_val1.to_csv("../../../data/kyungho/Claude3.7/{dataset}/{task}/1116/{task}-val-{i}-1.csv", index=False)
            df_tst1.to_csv("../../../data/kyungho/Claude3.7/{dataset}/{task}/1116/{task}-test-{i}-1.csv", index=False)
        except Exception as e:
            print("[feat1-error]", e)
        ...
        try:
            ...
            df_trn2.to_csv("../../../data/kyungho/Claude3.7/{dataset}/{task}/1116/{task}-train-{i}-2.csv", index=False)
            df_val2.to_csv("../../../data/kyungho/Claude3.7/{dataset}/{task}/1116/{task}-val-{i}-2.csv", index=False)
            df_tst2.to_csv("../../../data/kyungho/Claude3.7/{dataset}/{task}/1116/{task}-test-{i}-2.csv", index=False)
        except Exception as e:
            print("[feat2-error]", e)
        ...
        try:
            ...
            df_trn3.to_csv("../../../data/kyungho/Claude3.7/{dataset}/{task}/1116/{task}-train-{i}-3.csv", index=False)
            df_val3.to_csv("../../../data/kyungho/Claude3.7/{dataset}/{task}/1116/{task}-val-{i}-3.csv", index=False)
            df_tst3.to_csv("../../../data/kyungho/Claude3.7/{dataset}/{task}/1116/{task}-test-{i}-3.csv", index=False)
        except Exception as e:
            print("[feat3-error]", e)
...
"""
        ...
        output_file = f'../Claude3.7_{args.dataset}_{args.task}{args.n_iter}-1116-iter{i}.py'
        ...
        kept_in_iter = False
        for k in range(1, 4):
            train_path = f"../../../data/kyungho/Claude3.7/{dataset}/{task}/1116/{task}-train-{i}-{k}.csv"
            val_path   = f"../../../data/kyungho/Claude3.7/{dataset}/{task}/1116/{task}-val-{i}-{k}.csv"
            test_path  = f"../../../data/kyungho/Claude3.7/{dataset}/{task}/1116/{task}-test-{i}-{k}.csv"
            ...
            #fewshot_text = fewshot_block_simple(preds_df, entity_col=entity_col, n_each=3)
            #print(fewshot_text)
            feature_col = feature_corr_mi(train_df, target_col="target", key_col=entity_col)
            ...
            if new_auc >= auroc_prev:
                ...
                #resp_success = client.messages.create(
                #    model=model_ids[0],
                #    system="You are a data scientist for prediction task in relational database.",
                #    messages=base_msgs+[{"role":"user","content":[{"type":"text","text": success_reason}]}],
                #    max_tokens=2000,
                #    temperature=args.temp,
                #)
                #resp_succ_text = to_single_text_block(resp_success.content)
                ...
                #resp_fail = client.messages.create(
                #    model=model_ids[0],
                #    system="You are a data scientist for prediction task in relational database.",
                #    messages=base_msgs+[{"role":"user","content":[{"type":"text","text": fail_reason}]}],
                #    max_tokens=2000,
                #    temperature=args.temp,
                #)
                #resp_fail_text = to_single_text_block(resp_fail.content)
                print("=== Fail Reason ===")
    ...
    df_train.to_csv(os.path.join(ra_dir, f"{args.task}-train-1116-{args.n_iter}-{args.temp}.csv"), index=False)
    df_val.to_csv(os.path.join(ra_dir, f"{args.task}-valid-1116-{args.n_iter}-{args.temp}.csv"), index=False)
    df_tst.to_csv(os.path.join(ra_dir, f"{args.task}-test-1116-{args.n_iter}-{args.temp}.csv"), index=False)
    ...
    final_path = os.path.join(ra_dir, f"{args.task}-train-1116-{args.n_iter}-{args.temp}.csv")
    print(final_path)
